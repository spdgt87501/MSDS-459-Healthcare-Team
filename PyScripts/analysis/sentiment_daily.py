...
# It aggregates article sentiment into a daily sentiment ratio (proportion of positive articles)
# It calculates the average daily return across all analyzed stock tickers
# It then calculates the correlation between the sentiment and the stock market performance
# It then saves the data to a CSV file - daily_market_sentiment_with_returns.csv
...
# Aggregate by day
daily_sentiment = df.groupby(df["publish_date"].dt.date).agg(
...
print("\nSentiment date range:", daily_sentiment["publish_date"].min(), "to", daily_sentiment["publish_date"].max())
...
print("\nAverage daily returns sample:")
print(daily_avg_return.head())

# Merge with sentiment data
merged = pd.merge(daily_sentiment, daily_avg_return, left_on="publish_date", right_on="date", how="inner")
...
print(f"Number of days: {len(daily_sentiment)}")
print(f"Number of stocks analyzed: {len(stock_files)}")

# Calculate correlation between sentiment and returns
correlation = merged["sentiment_ratio"].corr(merged["avg_daily_return"])
print(f"\nCorrelation between sentiment ratio and average daily returns: {correlation:.3f}")

# Save to CSV for easy viewing
output_path = os.path.join(output_dir, "daily_market_sentiment_with_returns.csv")
